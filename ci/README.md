...
2  browser   build/cache  (forks, non-PR runs, or native source changes)
             fetch        (official repositories, driver-only PRs)
3a smoke     patch guards, build-tester                 ~15 min
3b full      Playwright x2, leaks, stealth              ~40 min
4  gate      the required check
```

**Only official repositories use the driver-only download shortcut.** In
`daijro/camoufox` and `camoufox/camoufox`, a PR with no browser source changes
uses `fetch-browser` to download the published release.

**Forks test their own branch's browser.** A fork can already contain native
changes before a Python-only PR starts. The official release would omit those
changes, so every fork PR selects the source build/cache path. Matching native
inputs still reuse the compiled browser; the branch's Juggler resources are
overlaid as described below. Non-PR runs, including manual runs of `Tests`,
also use this path.

The separate manual packaging workflow, `.github/workflows/build.yml`, is
unchanged. Selecting a ref there builds that ref's patches and additions.
...
**A browser that is already built is not built again.** On official PRs,
`browser_changed` is computed against the pull request's *base*, so it stays
true for every push to a branch that touched `patches/` even once. On forks it
is always true. Both cases need the branch's browser, but neither requires
recompiling identical inputs to fix a typo in `ci/`.
...
The runner passes the explicitly selected binary to each guard using its
standalone CLI:

| Guard | Arguments supplied by the runner |
| --- | --- |
| `window-resolution-windows` | `--executable BINARY --report-dir OUTPUT` |
| `webgpu-windows` | `BINARY --output OUTPUT` |
| `fingerprint-persistence-windows` | `BINARY --output OUTPUT` |
| `fingerprint-seed-contexts` | `BINARY --config PROFILE --output OUTPUT/results.json` |

`OUTPUT` is `EVIDENCE_DIR/patch-output/GUARD_NAME`. The other guards retain
their no-argument CLI and receive `CAMOUFOX_EXECUTABLE_PATH`. All guards import
the checked-out `pythonlib`.

The three `*-windows` guards require Windows and record explicit skips, with
the platform and reason, on other hosts. A selection containing only
inapplicable guards returns an error. Unknown `--only` names also return an
error, so misspellings cannot silently reduce coverage.

The context guard uses the committed
`tests/patches/assets/fingerprint-seed-profile.json` by default. Override it
with `--seed-profile PATH` when replaying another fixed profile. For example,
from a Windows shell:

```text
python -m ci.run_patch_guards --binary C:/build/camoufox.exe --evidence-dir .ci-work/windows --only window-resolution-windows webgpu-windows fingerprint-seed-contexts
```

The persistence guard compares full exported bytes, website hashes and report
fields across normal restarts. Website ads are an existing dynamic exclusion:
marked Google ad chips are excluded from rendered cell text, while the raw
cell text and excluded ad text remain in the evidence. Identity fields and
complete hashes are still compared without truncation.
