...
已合并功能分支在确认其提交被远端自用分支完整包含后清理。
旧 `codex/publish-personal-artifacts`（`41da280`）仅包含一次性的旧产物
发布流程，按用户明确要求单独清理，不将该流程合入自用分支。
长期保留 `main` 和 `personal-use`；删除临时分支不删除已有 Release 或标签。
